...
export default function ConversationDetail() {
    ...
    useEffect(() => {
        const fetchBlockStatus = async () => {
            ...
            try {
                const response =
                    await getConversationWithBlockStatus(conversationId);
        ...
        if (socket) {
            const handleUserBlocked = (data) => {
                if (
                    data.blocker_id === currentUserId &&
                    data.blocked_id === currentConversation?.otherUserId
                ) {
                    setIsBlocking(true);
                } else if (
                    data.blocker_id === currentConversation?.otherUserId &&
                    data.blocked_id === currentUserId
                ) {
                    setIsBlocked(true);
                }
            };
    ...
    return (
        <div className="h-screen flex">
            ...
            <div className="flex-1 flex overflow-hidden relative">
                {!convIdParam || isNaN(conversationId) ? (
                    <WelcomeScreen />
                ) : (
                    <>
                        {/* CHAT */}
